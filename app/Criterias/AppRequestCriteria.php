...
class AppRequestCriteria extends RepositoryRequestCriteria
{
    ...
    public function apply($model, RepositoryInterface $repository): mixed
    {
        ...
        if ($search && is_array($fieldsSearchable) && count($fieldsSearchable)) {
            $searchFields = is_array($searchFields) || is_null($searchFields) ? $fieldsSearchable : explode(
                ';',
                $searchFields
            );
            ...
            $model = $model->where(
                function ($query) use ($fields, $search, $searchData, $isFirstField, $modelForceAndWhere, $fieldRules) {
                    // @var Builder $query
                    foreach ($fields as $field => $condition) {
                        $fieldName = is_numeric($field) ? $condition : $field;

                        if (!empty($fieldRules[$fieldName]) && !$this->canSearch(
                                $fieldName,
                                $search,
                                $fieldRules[$field]
                            )) {
                        ...
                        if ($isFirstField || $modelForceAndWhere) {
                            if (!is_null($value)) {
                                if (!is_null($relation)) {
                                    $query->whereHas(
                                        $relation,
                                        function ($query) use ($field, $condition, $value, $parsedValue) {
                                            if (!strcmp($value, $parsedValue)) {
                                                $query->where($field, $condition, $value);
                                            } else {
                                                $query->where(
                                                    function ($q) use ($field, $condition, $value, $parsedValue) {
...
//                \Log::info($model->toSql());
     ...
     * @return mixed
     */
    protected function canSearch($field, $search, $rules): mixed
    {
